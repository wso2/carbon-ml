 ...
 */
@Path("/datasets")
public class DatasetApiV11 extends MLRestAPI {

    private static final Log logger = LogFactory.getLog(DatasetApiV11.class);
    /*
    ...
    /**
     * Get the sample points of the latest dataset version.
     *
     * @param datasetId ID of the dataset.
     * @return JSON of {@link org.wso2.carbon.ml.commons.domain.SamplePoints} object
     */
    @GET
    @Path("/{datasetId}/latest-sample")
    @Produces("application/json")
    public Response getLatestSamplePoints(@PathParam("datasetId") long datasetId) {
        PrivilegedCarbonContext carbonContext = PrivilegedCarbonContext.getThreadLocalCarbonContext();
        int tenantId = carbonContext.getTenantId();
        String userName = carbonContext.getUsername();
        try {
            String samplePoints = datasetProcessor.getLatestSamplePoints(tenantId, userName, datasetId);
            if (samplePoints == null) {
                return Response.status(Response.Status.NOT_FOUND).build();
            }
            return Response.ok(new MLResponseBean(samplePoints)).build();
        } catch (MLDataProcessingException e) {
            String msg = MLUtils
                    .getErrorMsg(
                            String.format(
                                    "Error occurred while retrieving the latest sample set for the dataset [id] %s of tenant [id] %s and [user] %s .",
                                    datasetId, tenantId, userName), e);
            logger.error(msg, e);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new MLErrorBean(e.getMessage()))
                    .build();
