...
public class DatabaseHandler {
	 ...
	 *
	 * @return A Singleton DatabaseHandler object
	...
	protected static DatabaseHandler getDatabaseHandler() throws DatabaseHandlerException {
		try {
			...
			throw new DatabaseHandlerException("An error occured while connecting to database: " +
					e.getMessage(), e);
		}
	}
	...
	protected String getDatasetUrl(String datasetID) throws DatabaseHandlerException {
		...
		try {
			...
			// if the path for the given data-set exists.
			if (result.first()) {
			...
			throw new DatabaseHandlerException("An error occured while reading the Dataset " +
					datasetID + " from the database: " + e.getMessage(),
					e);
		} finally {
	...
	protected void insertDatasetDetails(String datasetID, String filePath, String projectID)
		...
		try {
			...
			throw new DatabaseHandlerException(
			                                   "An error occured while inserting details of dataset " +
			                                		   datasetID +
			                                		   " to the database: " +
			                                		   e.getMessage(), e);
		} finally {
	...
	protected void updateDataType(String featureName, String workflowID, String featureType)
		...
		try {
			...
			MLDatabaseUtil.rollBack(connection);
			throw new DatabaseHandlerException(
			                                   "An error occured while updating the data type of feature \"" +
			                                		   featureName +
			                                		   "\" of workflow " +
	 ...
	 * @param type
	 *            Array of data-types of each feature
	 * @param graphFrequencies
	 *            List of Maps containing frequencies for graphs, of each
	 *            feature
	 * @param missing
	 *            Array of Number of missing values in each feature
	 * @param unique
	 *            Array of Number of unique values in each feature
	 * @param descriptiveStats
	 *            Array of descriptiveStats object of each feature
	 * @throws DatabaseHandlerException
	 */
	protected void updateSummaryStatistics(String datasetID, String[] header, FeatureType[] type,
	                                       ...
	                                       Boolean include) throws DatabaseHandlerException {
		Connection connection = null;
		PreparedStatement updateStatement = null;
		try {
			...
			for (int column = 0; column < header.length; column++) {
				...
				summaryStat =
						createJson(type[column], graphFrequencies.get(column),
						           missing[column], unique[column],
						           descriptiveStats.get(column));
			...
			MLDatabaseUtil.rollBack(connection);
			throw new DatabaseHandlerException(
			                                   "An error occured while updating the database with summary statistics of the dataset " +
	 ...
	 *            Unique Identifier of the data-set
	 * @param datasetSample
	 *            SamplePoints object of the data-set
	 * @throws DatabaseHandlerException
	 */
	protected void updateDatasetSample(String datasetID, SamplePoints datasetSample)
		...
		try {
			...
			MLDatabaseUtil.rollBack(connection);
			throw new DatabaseHandlerException(
			                                   "An error occurred while updating the sample points of dataset " +
			                                		   datasetID + ": " + e.getMessage(), e);
		} finally {
			// enable auto commit.
			MLDatabaseUtil.enableAutoCommit(connection);
	...
	private SamplePoints getDatasetSample(String datasetID) throws DatabaseHandlerException {
		...
		try {
			...
			MLDatabaseUtil.rollBack(connection);
			throw new DatabaseHandlerException(
			                                   "An error occured while retrieving the sample of dataset " +
			                                		   datasetID + ": " + e.getMessage(), e);
		} finally {
			// close the database resources.
	 ...
	 * @param graphFrequencies
	 *            Bin frequencies of the column
	 * @param missing
	 *            Number of missing values in the column
	 * @param unique
	 *            Number of unique values in the column
	 * @param descriptiveStats
	 *            DescriptiveStats object of the column
	 * @return
	 */
	private JSONObject createJson(FeatureType type, SortedMap<?, Integer> graphFrequencies,
	...
	protected List<Feature> getFeatures(String datasetID, String workflowID, int startIndex,
		...
		try {
			...
			while (result.next()) {
				...
				if (ImputeOption.REPLACE_WTH_MEAN.toString().equalsIgnoreCase(result.getString(5))) {
			...
			throw new DatabaseHandlerException(
			                                   "An error occured while retrieving features of the data set: " +
			                                		   datasetID + ": " + e.getMessage(), e);
		} finally {
			// close the database resources.
	...
	protected List<String> getFeatureNames(String workflowID, String featureType)
		...
		try {
			...
			throw new DatabaseHandlerException(
			                                   "An error occurred while retrieving feature names of the dataset for workflow: " +
			                                		   workflowID + ": " + e.getMessage(), e);
		} finally {
			// close the database resources.
	...
	protected String getSummaryStats(String datasetID, String featureName)
		...
		try {
			...
			throw new DatabaseHandlerException(
			                                   "An error occured while retireving summary statistics for the feature \"" +
			                                		   featureName +
			                                		   "\" of the data set " +
			                                		   datasetID + ": " + e.getMessage(), e);
		} finally {
	...
	protected int getFeatureCount(String datasetID) throws DatabaseHandlerException {
		...
		try {
			...
			throw new DatabaseHandlerException(
			                                   "An error occured while retireving feature count of the dataset " +
			                                		   datasetID + ": " + e.getMessage(), e);
		} finally {
			// close the database resources
			MLDatabaseUtil.closeDatabaseResources(connection, getFeatues, result);
