...
CONSTRAINT fk_Dataset_Feature FOREIGN KEY(Dataset) REFERENCES ML_Dataset(ID) ON UPDATE CASCADE
);

CREATE TABLE IF NOT EXISTS ML_Model(
Id INTEGER NOT NULL auto_increment,
Accuracy DECIMAL(8, 4),
Hyper_Parameters VARCHAR(500),
Feature_Importance VARCHAR(2000),
...
CONSTRAINT fk_Dataset_Project FOREIGN KEY(Dataset) REFERENCES ML_Dataset(ID) ON UPDATE CASCADE
);
...
CONSTRAINT fk_Project_Process FOREIGN KEY(Project) REFERENCES ML_Project(ID) ON UPDATE CASCADE
);
...
CONSTRAINT fk_Model_Execution FOREIGN KEY(Model) REFERENCES ML_Model(ID) ON UPDATE CASCADE,
CONSTRAINT fk_Process_Execution FOREIGN KEY(Process) REFERENCES ML_Process(ID) ON UPDATE CASCADE
);
...
('LOGISTIC_REGRESSION','Classification',4,5,1,5,'{"parameters":[{"parameter":"Iterations","value":100},{"parameter":"Learning_Rate","value":0.001},{"parameter":"Reg_Type","value":"L1"},{"parameter":"Reg_Parameter","value":0.001}]}'),
