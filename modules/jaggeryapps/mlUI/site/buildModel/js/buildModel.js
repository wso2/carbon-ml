$('document').ready(function() {
    $('#allAlgosBackButton').click(function() {
        jagg.sessionAwareJs();
        loadQuestions();
    });

    $('#recommendedAlgosBackButton').click(function() {
        jagg.sessionAwareJs();
        loadQuestions();
    });

    $('#questionsBackButton').click(function() {
        jagg.sessionAwareJs();
        loadAlgoTypes();
    });
});
...
// When the alogrithm type is selelcted
...
// When continue from questions
$('#questionsContinueButton').click(function() {
    jagg.sessionAwareJs();
...
// Check whether hyperparameter values are valid.
function hasValidHyperparameters() {
    var validity = true;
    var parameter = $('.parameterValue');
    for (var i=0 ; i<parameter.length ; i++) {
        var value = parameter[i].getElementsByTagName("input")[0].value.trim();
        parameter[i].getElementsByTagName("div")[0].style.display = 'none';
        if(!value || value == "") {
            parameter[i].getElementsByTagName("div")[0].style.display = 'inline';
            validity = false;
        }
    }
    return validity;
...
// Submit and build model
$('#algorithmsContinueButton').click(function() {
    jagg.sessionAwareJs();
    var isModelExecStarted = false;
    // Check whether an algorithm is selected
    if(!$('input[name=algorithm]:checked')[0]) {
        $('.algoSelectionErrorMessage').css('display', 'block');
    } else if (hasValidHyperparameters()) {
        $.ajax({
        ...
        if (!isModelExecStarted) {
            ...
            if (algoName == undefined) {
                ...
                while (parameters[i] != undefined) {
                    ...
                    parameter.push(values[i].value.trim());
