...
dw.jq = function (e) {
    return jQuery_1_4_2(document.createElement(e))
}
...
dw.regex = function () {
    ...
    r.candidates = function (records, o) {
        ...
        if (records.length) {
            ...
            if (before === undefined || before.length === 0) {
                between = after.map(function (a) {
                    return {after: a, on: /.*/}
                });
            }
            else if (after === undefined || after.length === 0) {
                between = before.map(function (a) {
                    return {before: a, on: /.*/}
                });
            }
            else {
    ...
    r.parse = function (str, startCursor, endCursor, o) {
        ...
        var o = o || {}, code, records, startIndex, endIndex, index = 0,
            on, before, after,
            matchAfter = o.matchAfter || 3, matchBefore = o.matchBefore || 3;
        /*candidates*/
        ...
        return {
            on: on.map(collapse), after: (after || []).map(function (x) {
                return collapse(x.reverse())
            }), before: (before || []).map(function (x) {
                return collapse(x)
            })
        }
    ...
    for (i = 0; i < splits.length; ++i) {
        if (i % 2 === 1) {
            ...
            if (occurrence === which) {
                ...
                newSplits.push({
                    start: prefix.length,
                    end: prefix.length + splits[i].value.length,
        ...
        if (positions.length == 2) {
            if (value.length >= positions[1]) {
                ...
                splits.push({
                    ...
                    value: value.substr(split_start, split_end - split_start)
                })
        ...
        if (t._positions && t._positions.length) {
            ...
            for (var v = 0; v < values.length; ++v) {
                ...
                var params = {
                    ...
                    after: t._after,
                    on: t._on,
                    ignore_between: t._ignore_between
                }
...
dw.fold = function (column) {
    ...
    t.description = function () {
        return [
            ...
            dw.column_clause(t, t._column, 'column'),
            ' using ',
            dw.key_clause(t, t._keys.map(function (c) {
                return c === -1 ? 'header' : c
            }), 'keys', {
                editor_class: 'fold', clean_val: function (x) {
                    return Number(x)
                }
            }),
...
dw.map = function (column) {
    var t = dw.transform(column);
    dv.ivar(t, [{name: 'result', initial: dw.COLUMN}, {name: 'update', initial: false}, {
...
dw.merge = function (column) {
    ...
    t.transform = function (values) {
        ...
        var v = values.filter(function (v) {
            return v != undefined
        }).join(glue)
...
dw.wrap = function (row) {
    ...
    t.apply = function (tables, options) {
        ...
        for (var r = start_row; r < end_row; ++r) {
            if (row.test(table, r)) {
                effectedRows.push(r)
                dv.range(acc.length - data.length).forEach(function () {
                    data.push([])
                })
                if (acc.length) {
        ...
        return {
            effectedRows: effectedRows, keyCols: dv.range(cols).map(function (c) {
                return table[c]
            })
        }
    }
...
dw.reduce = function (column) {
    ...
    t.apply = function (tables) {
        ...
        var x = table.query({
            dims: names, vals: t._measures.map(function (m) {
                return dv.first(m)
            })
        });
...
dw.row = function (conditions) {
    ...
    t.description = function () {
        ...
        return [
            ' rows where ' + t._conditions.map(function (c) {
...
dw.row.fromFormula = function (formula) {
    ...
    preds = preds.map(function (pred) {
        ...
        if (index = pred.indexOf('index in (') != -1) {
            ...
            indices = indices.split(/,/g).map(function (i) {
                return Number(i) - 1
            });
...
dw.rowIndex = function (indices) {
    ...
    t.description = function (o) {
        ...
        if (simple) {
            ...
            return (indices.length === 1 ? (indices[0] === -1 ? '' : 'row ') : 'rows ') + indices.map(function (i) {
                    return i === -1 ? 'header' : (i + 1)
                }).join(',')
        }
        else {


            return 'index in (' + indices.map(function (i) {
                    return i + 1
                }).join(',') + ')'
        }
    }
...
dw.split = function (column) {
    ...
    t.transform = function (values) {
        if (t._positions && t._positions.length) {
            ...
            var params = {
                ...
                after: t._after,
                on: t._on,
                ignore_between: ignore_between || t._ignore_between
            }
    ...
    t.description = function (table) {
        ...
        if (Number(t._max) === 0) {
            ...
            description = description.concat(dw.select_clause(t, {
...
dw.textPattern = function (column) {
    var t = dw.map(column);
    dv.ivar(t, [
        {name: 'on', initial: undefined}, {name: 'before', initial: undefined}, {
            name: 'after',
            initial: undefined
        }, {name: 'ignore_between', initial: undefined},
...
dw.transform = function (column) {
    ...
    dv.ivar(t, [{name: 'table', initial: 0}, {name: 'status', initial: dw.status.active}, {
        name: 'drop',
        initial: false
    }])
    ...
    t.params = function () {
        return dv.keys(t).filter(function (k) {
            return k[0] === '_'
        })
    }
    ...
    t.valid_columns = function (tables) {
        var table = t.getTable(tables);
        var columns = t.columns(table).filter(function (c) {
            return c === undefined
        });
        if (columns.length) {
...
dw.unfold = function (column) {
    ...
    t.apply = function (tables, options) {
        ...
        var table = t.getTable(tables),
            columns = t.columns(table),
            toHeaderNames = columns.map(function (c) {
                return c.name
            }),
            keyColumns = table.filter(function (c) {
                return toHeaderNames.indexOf(c.name) === -1 && t._measure != c.name
            }),
        ...
        for (var r = start_row; r < end_row; r++) {
            var key = keyColumns.map(function (e) {
                return e[r];
            }).join('*');
        ...
        return {
            toKeyRows: [-1], toHeaderCols: columns, toValueCols: [valueCol], valueCols: valueCols.map(function (c) {
                return table[c]
            }).filter(function (c) {
                return c != undefined
            })
...
dw.preview = function (vtable, table, transform, afterTable, tableSelection) {
    ...
    if (transform) {
        ...
        var tstats = transform.apply([sample], {max_rows: 1000, start_row: start_row, end_row: end_row}),
            ...
            splits = valueStats.map(function (v) {
                if (v) return v[0] && v[0].splits
            })
        ...
        transform.column().forEach(function (col) {
            ...
            children = jQuery_1_4_2('.updatedTable td:nth-child(' + (colIndex + 2) + ')');
        ...
        newCols.forEach(function (col) {
            jQuery_1_4_2('.updatedTable td:nth-child(' + (col.index() + 2) + ')').addClass('previewNew').addClass('unclickable');
        })

        droppedCols.forEach(function (col) {
            jQuery_1_4_2('.updatedTable td:nth-child(' + (col.index() + 2) + ')').addClass('previewDeleted');
        })

        toHeaderCols.forEach(function (col) {
            colIndex = table[col.name].index()
            jQuery_1_4_2('.previewBeforeTable td:nth-child(' + (colIndex + 2) + ')').addClass('previewSchema');
        ...
        toValueCols.forEach(function (col) {
            colIndex = table[col.name].index();
            jQuery_1_4_2('.previewBeforeTable td:nth-child(' + (colIndex + 2) + ')').addClass('previewNew');

            vtable.rows(function (r, i) {
                if (i === -1) jQuery_1_4_2('.previewBeforeTable tr:nth-child(2) th:nth-child(' + (colIndex + 2) + ')').addClass('previewKey');
        ...
        valueCols.forEach(function (col) {
            ...
            jQuery_1_4_2('.previewAfterTable td:nth-child(' + (colIndex + 2) + ')').addClass('previewNew');


            afterTable.rows(function (r, i) {
                if (i === -1) jQuery_1_4_2('.previewAfterTable tr:nth-child(2) th:nth-child(' + (colIndex + 2) + ')').addClass('previewKey');
                else r.find('td:nth-child(' + (colIndex + 2) + ')').addClass('previewKey')
            }, toKeyRows)
        ...
        keyCols.forEach(function (col) {
            colIndex = sample[col.name].index();
            jQuery_1_4_2('.previewAfterTable td:nth-child(' + (colIndex + 2) + ')').addClass('previewKey');
    ...
    jQuery_1_4_2('.unclickable').unbind('mouseup').unbind('mousedown')
}
...
dw.vtable = function (container, options) {
    container = jQuery_1_4_2(container);
    options = options || {}
    ...
    vtable.draw = function () {
        ...
        otable = jQuery_1_4_2('#' + containerID).dataTable({
            "aaData": data,
            "aoColumns": columns,
            "aaSorting": [],
        ...
        container.find('td').mouseup(function (e) {
            ...
            var colIndex = jQuery_1_4_2(this).index();
            if (colIndex) {

                if (this === mousedownCell) {
                    var selection = getSelection();
                    var rowIndex = jQuery_1_4_2(this).parent();
            ...
            jQuery_1_4_2(e.currentTarget).empty().append(input)
        ...
        container.find('.dataTables_scrollHead th').click(function (e) {
            ...
            setTimeout(function () {
                if (!dblClick) {
                    ...
                    interaction({
                        type: dw.engine.col,
                        position: {row: -1, col: table[jQuery_1_4_2(me).index() - 1].name},
                        table: table,
                        shift: e.shiftKey,
                        ctrl: e.metaKey
                    })
            ...
            var input = dw.jq('input'), me = this;
            jQuery_1_4_2(e.currentTarget).empty().append(input)
            input.css('width', '100%').css('height', '16px').focus().val(dw.display_name(table[jQuery_1_4_2(me).index() - 1].name))
            input.blur(function () {
                if (dw.display_name(table[jQuery_1_4_2(me).index() - 1].name) != dw.display_name(input.val())) {
                    vtable.save_settings();
                    onexecute(dw.set_name(table[jQuery_1_4_2(me).index() - 1].name, [input.val()]))
                }
            }).keydown(function (e) {
                if (e.which === 13) {
                    if (dw.display_name(table[jQuery_1_4_2(me).index() - 1].name) != dw.display_name(input.val())) {
        ...
        var scrollContainer = jQuery_1_4_2('.ui-layout-center');
        ...
        jQuery_1_4_2('.ui-layout-center').scroll(function () {
            vtable.save_settings();
        })
    ...
    vtable.save_settings = function () {
        ...
        var scrollContainer = jQuery_1_4_2('.ui-layout-center');
        settings['scrollLeft'] = jQuery_1_4_2(scrollContainer).scrollLeft();
    }
    ...
    vtable.add_sort_status = function () {
        ...
        if (options.wrangler) {
            var sortColumn;
            var lastSort = options.wrangler.filter(function (t) {
                return t.active() && t.name === dw.transform.SORT
            });
            lastSort = lastSort[lastSort.length - 1];
            if (lastSort) {
                sortColumn = lastSort.column();
                container.find('.column_sort_status').each(function (c, i) {
                    if (table[jQuery_1_4_2(this).parent().index() - 1] === table[sortColumn]) {
                        if (lastSort.direction()[0] === 'desc') {
                            jQuery_1_4_2(this).removeClass('column_sort_unsorted').addClass('column_sort_desc').click(function (e) {
                                e.stopPropagation();
                                e.stopImmediatePropagation();
                                onexecute(dw.sort(table[jQuery_1_4_2(this).parent().index() - 1].name).direction('asc').as_type([table[jQuery_1_4_2(this).parent().index() - 1].wrangler_type]))
                            })
                        }
                        else {
                            jQuery_1_4_2(this).removeClass('column_sort_unsorted').addClass('column_sort_asc').click(function (e) {
                                e.stopPropagation();
                                e.stopImmediatePropagation();
                                onexecute(dw.sort(table[jQuery_1_4_2(this).parent().index() - 1].name).direction('desc').as_type([table[jQuery_1_4_2(this).parent().index() - 1].wrangler_type]))
                            })
                        }
                    }
                })
            ...
            container.find('.column_sort_unsorted').click(function (e) {
                e.stopPropagation();
                e.stopImmediatePropagation();
                onexecute(dw.sort(table[jQuery_1_4_2(this).parent().index() - 1].name).as_type([table[jQuery_1_4_2(this).parent().index() - 1].wrangler_type]))
            });
        }
    }
    ...
    var draw_type_icons = function () {
        ...
        vtable.header_row().slice(1).each(function (x, i) {
            ...
            jQuery_1_4_2(i).addClass(className).addClass('icon')
        })
    }
    ...
    var draw_summary = function (container, col) {
        ...
        var bars = vis.selectAll("g.bar")
            ...
            .attr("transform", function (d, i) {
                return "translate(" + normalized_summary.slice(0, i).reduce(function (s, x) {
                        return s + x.val
                    }, 0) + ",0)";
            });
    ...
    var normalize_summary = function (col, summary, min, width) {
        ...
        nonzero = dv.keys(summary).filter(function (k) {
            return k != 'unique'
        }).filter(function (key) {
    ...
    vtable.rows = function (cb, rows) {
        ...
        rows.filter(function (r) {
            ...
            if (r === -1) return cb(jQuery_1_4_2('#' + containerID + ' thead tr:nth-child(' + (2) + ')'), r);

            index = (r + 1) % vtable.page_length();
            if (index === 0) index = vtable.page_length();

            return cb(jQuery_1_4_2('#' + containerID + ' tr:nth-child(' + (index) + ')'), r);
        })
    }
    ...
    vtable.cols = function (cb, cols) {
        ...
        cols.forEach(function (c) {
            cb(jQuery_1_4_2('#' + containerID + ' td:nth-child(' + (c + 2) + ')'), c)
        })
    }
...
\n"
    ...
    return preamble + wrangler + w.filter(function (t) {
            return t.active()
        }).map(function (t) {
            var comment = '\n\/* ' + t.comment() + ' *\/\n';
...
dw.wrangler_export.python = function (w) {
    ...
    var parse_python_transform = function (t) {
        ...
        var n = t.name.split('_').map(function (c) {
        ...
        }).join(',\n' + dv.range(0, 10 + n.length).map(function () {
            return ' '
        }).join(''))
...
dw.corpus = function () {
        ...
        switch (transform.name) {
            ...
            case dw.transform.FOLD:
                ...
                if (given_params.indexOf('column') === -1) {
                    return [transform.clone()].concat(get_columns(table, [dw.integer, dw.string]).map(function (c) {
                        return transform.clone().column(c)
                    }))
                }
            ...
            case dw.transform.UNFOLD:
                if (given_params.indexOf('measure') === -1) {
                    ...
                    return get_columns(table, [dw.integer, dw.number, dw.string]).map(function (c) {
                        return transform.clone().measure(c)
                    })
    ...
    get_columns = function (table, type_hierarchy, count) {
        ...
        return cols.slice(0, count).map(function (c) {
            return c.name
        })
    }
...
dw.engine = function (options) {
    ...
    engine.run = function (k) {
        ...
        if (enable_proactive &&
            ...
            var foldColNames = engine._table.slice_cols(1, engine._table.cols()).map(function (e) {
                return e.name
            });
            ...
            if (nCols >= 3 && nCols <= 5) {
                ...
                for (var c1 = 0; c1 < nCols; c1++) {
                    for (var c2 = 0; c2 < nCols; c2++) {
                        ...
                        var otherCols = otherColsIndices.map(function (c) {
                            return engine._table[c]
                        });
                        ...
                        if (otherColsSummaries.filter(function (e) {
                                return (e.missing.length > 0)
                            }).length > 0) {
                            continue;
                        }
            ...
            params = scoredCandidates.map(function (e) {
                return e[1];
            });
    ...
    var inferTransforms = function (params) {
        ...
        if (workingTransform) {
            tset = tset.filter(function (t) {
                return t.name != workingTransform.name
            })
            tset.unshift(workingTransform)
        }

        transforms = transforms.concat(tset.reduce(function (tforms, t) {
            return tforms.concat(params.filter(function (p) {
                return !p.is_transform
            }).reduce(function (acc, p) {
                return acc.concat(inferTransformParameterSet(t, p));
            }, []))
        }, []))

        transforms = params.filter(function (p) {
            return p.is_transform
        }).concat(transforms)
    ...
    var sortTransforms = function (transforms) {
        ...
        transforms.sort(function (a, b) {
            ...
            if (aw.td > bw.td) return -1;
            if (bw.td > aw.td) return 1;
            if (aw.tf > bw.tf) return -1;
            if (bw.tf > aw.tf) return 1;
            if (aw.tdl < bw.tdl) return -1;
            if (bw.tdl < aw.tdl) return 1;
            return 0;
        })
    ...
    var inferTransformParameterSet = function (transform, param) {
        ...
        neededParams = t.enums().filter(function (x) {
            return keys.indexOf(x) === -1
        })
        var top = corpus.top_transforms({
            transform: t,
            given_params: keys,
            needed_params: neededParams,
            table: engine._table
        })
        ...
        if (promoted && promoted === t.name) {
            ...
            return top.slice(0, 30).filter(function (x) {
                return x.well_defined(engine._table)
            })
        }
    ...
    var getSelectionRecords = function (inputs) {
        ...
        if (inputs && inputs.length) {
            ...
            return inputs.map(function (input, i) {
                ...
                return {
                    type: dw.engine.highlight,
                    params: dw.regex.record(text, start, end, table[col].name, row, table)
                };
            })
        }
...
dw.edit_inference = function () {
    ...
    r.candidates = function (table, records, o) {
        ...
        if (records.length) {
            ...
            candidates.push({
                column: table[record.position.col].name,
                to: record.val,
                row: dw.row(dw.rowIndex([record.position.row]))
            });
...
dw.infer_type_transforms = function (table) {
    ...
    return [dw.set_type(cols.map(function (c) {
        return c.name
    }), types), dw.set_role(cols.map(function (c) {
        return c.name
    }), roles)]
...
dw.clause = function (options) {
    ...
    var clause = {}, element = jQuery_1_4_2(document.createElement('span'));
...
dw.combo_with_other = function (select_options, options) {
    ...
    editor = dw.jq('select');
    if (!select_options.filter(function (c) {
            return c.value === selected_val
        }).length) {
...
dw.column_clause = function (t, columns, param, options) {
    ...
    clause.editor = function (draw_options) {
        ...
        if (draw_options.table) {
            ...
            editor = jQuery_1_4_2(document.createElement('input'));
...
dw.key_clause = function (t, keys, param, options) {
    ...
    clause.value = function () {
        var vals = editor.val().replace(/ /g, '').replace('header', '-1').split(/,/g).map(function (c) {
            return c === 'header' ? c : Number(c) - 1
        });
        if (options.clean_val) {
            return vals.map(options.clean_val)
        }
...
dw.regex_clause = function (t, param, options) {
    ...
    clause.editor = function () {
        var val = regex ? regex.toString().replace(/^\/|\/$/g, '').replace(/\n/g, '\\n').replace(/\t/g, '\\t') : '';
        var regex_options = [{name: 'comma', value: ','}, {name: 'tab', value: '\\t'}, {
            name: 'whitespace',
            value: ' '
        }, {name: 'newline', value: '\\n'}, {name: 'pipe', value: '\\|'}, {name: 'period', value: '\\.'}]
        editor = dw.combo_with_other(regex_options, {
            val: val, ondone: function (val) {
...
dw.select_clause = function (t, options) {
    ...
    clause.description = function (container) {
        ...
        editor = jQuery_1_4_2(document.createElement('select'))
...
dw.detail_editor = function (container, transform, options) {
    ...
    editor.draw = function () {
        var clause, detail;
        transform.params().map(function (p) {
            return p.substr(1)
        })
...
dw.editor = function (container, suggestions, options) {
    ...
    editor.draw = function () {
        ...
        editorWrapper = jQuery_1_4_2(document.createElement('div')).attr('id', 'editorTransformContainer')
        ...
        suggestions.forEach(function (s, i) {
            ...
            var sWrapper = jQuery_1_4_2(document.createElement('div')).addClass('suggestion')
            ...
            if (s) {
                descriptionWrapper = jQuery_1_4_2(document.createElement('div')).addClass('descriptionEditor')
                addButton = jQuery_1_4_2(document.createElement('div')).addClass('editorButton addButton')
                ...
                dw.descriptionEditor(descriptionWrapper, s, {
                    onedit: onedit,
                    editable: (i === 0),
                    table: options.table
                }).draw()
                ...
                sWrapper.append(jQuery_1_4_2(document.createElement('div')).addClass('clear'));
                ...
                var hoverConfig = {
                    over: function (e) {
                        ...
                        select(jQuery_1_4_2(e.currentTarget));
                    },
                    timeout: 500,
                    out: function () {
                    }
        ...
        jQuery_1_4_2('.suggestion').filter(':not(.suggestion.selected)').fadeTo(500, 1);
        return editor;
    }

    editor.promote = function () {
        var fadeTime = 800, selected = editor.selected(), offset = selected.offset(),
            primary = jQuery_1_4_2('.suggestion:first'), primaryOffset = primary.offset(), top = primaryOffset.top - primary.height() - 85, width = selected.width();


        jQuery_1_4_2('.suggestion').filter(':not(.suggestion.selected)').fadeTo(fadeTime, 0);
        setTimeout(function () {
    ...
    editor.prev = function () {
        ...
        if (old.length) {
            ...
            select(jQuery_1_4_2('.suggestion:last'))
        }
    }
    ...
    editor.next = function () {
        ...
        if (old.length) {
            ...
            select(jQuery_1_4_2('.suggestion:first'))
        }

    }

    editor.first_suggestion = function () {
        select(jQuery_1_4_2('.suggestion:nth-child(2)'))
    }
    ...
    editor.selected = function () {
        return jQuery_1_4_2('.suggestion.selected')
    }
...
dw.script = function (container, wrangler, options) {
    ...
    script.draw = function () {
        ...
        editorWrapper = jQuery_1_4_2(document.createElement('div')).attr('id', 'scriptTransformContainer')
        container.append(editorWrapper)
        ...
        transforms.forEach(function (t, i) {
            ...
            if (!t.deleted()) {
                ...
                var tWrapper = jQuery_1_4_2(document.createElement('div')).addClass('scriptTransform')
                ...
                otherButton = jQuery_1_4_2(document.createElement('div'));
                ...
                toggleButton = jQuery_1_4_2(document.createElement('div')).addClass('editorButton').addClass('primaryButton')
                ...
                tWrapper.append(jQuery_1_4_2(document.createElement('div')).addClass('clear'))
        ...
        jQuery_1_4_2('.scriptTransform').mouseenter(function () {
            jQuery_1_4_2(this).addClass('selected')
        }).mouseleave(function () {
            jQuery_1_4_2(this).removeClass('selected')
        })
...
dw.tmenu = function (container, options) {
    ...
    var options = options || {}, interaction = options.interaction, transforms = options.transforms || [
            {
                name: 'Text',
                sub: [{name: 'Split', transform: dw.transform.SPLIT}, {
                    name: 'Cut',
                    transform: dw.transform.CUT
                }, {name: 'Extract', transform: dw.transform.EXTRACT}, {name: 'Edit', transform: dw.transform.CONVERT}]
            },
            {
                name: 'Columns',
                sub: [{name: 'Fill', transform: dw.transform.FILL}, {
                    name: 'Translate',
                    transform: dw.transform.TRANSLATE
                }, {name: 'Drop', transform: dw.transform.DROP}, {name: 'Merge', transform: dw.transform.MERGE}]
            },
            {
                name: 'Rows',
                sub: [{name: 'Delete', transform: dw.transform.FILTER}, {
                    name: 'Fill',
                    transform: dw.transform.FILL
                }, {name: 'Promote', transform: dw.transform.SET_NAME}]
            },
            {
                name: 'Table',
                sub: [{name: 'Fold', transform: dw.transform.FOLD}, {
                    name: 'Unfold',
                    transform: dw.transform.UNFOLD
                }, {name: 'Transpose', transform: dw.transform.TRANSPOSE}]
            },
            {name: 'Clear', sub: []}
        ];
    ...
    menu.draw = function () {
        ...
        transforms.forEach(function (m) {
            ...
            m.sub.forEach(function (s) {
                ...
                sub.append(title)
                jQuery_1_4_2(title).click(function (e) {
            ...
            if (m.name != 'Clear') {
                jQuery_1_4_2(list).click(function () {
                    var position = jQuery_1_4_2(this).position();


                    sub.css('left', position.left).css('top', position.top + 20);
                    sub.show();

                    jQuery_1_4_2(this).height(subheight)
                    jQuery_1_4_2(this).addClass('openMenu')


                }).mouseleave(function () {
                    sub.hide();
                    jQuery_1_4_2(this).height(19)
                    jQuery_1_4_2(this).removeClass('openMenu')
...
dw.transform_menu = function (container, options) {
    ...
    menu.draw = function () {
        ...
        sub.selectAll('div.menu_option')
            .data(function (d, i) {
                return d3.range(transforms[d].sub.length).map(function () {
                    return d
                })
            })
            .enter().append('div')
            .attr('class', function (d, i) {
                return 'menu_option ' + transforms[d].sub[i].name
            })
            .text(function (d, i) {
                return transforms[d].sub[i].name
            })
            .on('mousedown', function (d, i) {
                interaction({type: dw.engine.filter, transform: transforms[d].sub[i].transform})
            })

        jQuery_1_4_2(vis[0]).append(editor)
    ...
    menu.update = function (e) {
        editor.empty();
        jQuery_1_4_2('.selected_menu_option').removeClass('selected_menu_option')
        if (e.transform) {
            ...
            jQuery_1_4_2('.menu_option.' + name).addClass('selected_menu_option')
        }
...
dw.wrangler = function (options) {
    var tContainer = options.tableContainer, previewContainer = options.previewContainer, transformContainer = options.transformContainer, table = options.table, originalTable = table.slice(), temporaryTable, vtable, afterTable, transform,
        engine, suggestions, editor, wrangler = {}, script, w = dw.wrangle(), tableSelection, scriptContainer = jQuery_1_4_2(document.createElement('div')).attr('id', 'scriptContainer'), editorContainer = jQuery_1_4_2(document.createElement('div')).attr('id', 'editorContainer'), dashboardContainer = options.dashboardContainer;
    ...
    var transform_menu = dw.transform_menu(dashboardContainer, {
        interaction: interaction,
        onclear: clear_editor,
        onedit: interaction,
        table: undefined
    })
    ...
    function execute_transform(transform, params) {
        ...
        var x = jQuery_1_4_2('#scriptTransformContainer')
        x.scrollTop(100000);
    ...
    function drawEditor() {
        ...
        editor = dw.editor(editorContainer, suggestions, {
            onpromote: promote_transform,
            onhighlight: highlight_suggestion,
            onselect: execute_transform,
            onedit: interaction,
            table: table
        }).draw()
    }
    ...
    function exportTable() {
        ...
        select.change(function () {
            ...
            jQuery_1_4_2('.exportHeader').removeClass('selectedExportHeader');
            jQuery_1_4_2(this).addClass('selectedExportHeader')
            inputArea.focus();
            inputArea.select();
        ...
        jQuery_1_4_2('#profilerCenterPanel').prepend(upload)
        ...
        jQuery_1_4_2('.exportHeader:first').addClass('selectedExportHeader');
    ...
    function drawScript() {
        var scrollTop = jQuery_1_4_2('#scriptTransformContainer').scrollTop();
        ...
        jQuery_1_4_2('#scriptTransformContainer').scrollTop(scrollTop);
    }
    ...
    jQuery_1_4_2(document).bind('keydown', function (event) {
